...
date-modified: "23 September 2026"
execute: 
  echo: false
---
...
The response to the 2026 Nepal Floods is comprised of the efforts of 58 Lead Agencies and 50 Implementing Partners. According to the 5Ws dashboard, a total of 64,263 persons have been reached across 15 Municipalities. The Flash Appeal estimated 84,720 people were affected by the flooding. 

## Executive summary

* The ward is a more appropriate administrative level than the municipality for planning and analysis of a response along a narrow corridor.
...
* Bidur, Gosaikunda and Uttargaya are the municipalities to which the majority of response resources have been allocated to date. In relation to the amount of damage they suffered, Bidur and Uttargaya have received slightly more aid than Gosaikunda.

* Populations in Galchhi and Aamachhodingmo have received a high proportion of relief interventions in comparison to the scale of damage in these areas. Benighat Rorang and Siddhalek are the opposite, though being much further downstream, flood damage in these areas is likely less severe. 

The municipal-level plots below allow for the comparison between the number of impacted buildings, the sum of all beneficiary frequencies in the 5Ws (which includes double counting and is indicative of how humanitarian partners have allocated their resources to date) and the total population of the affected wards. 
...
consolidated |> 
  filter(response_modality != "Awareness/Advocacy") |> 
  mutate(district = ifelse(municipality == "Gosaikunda", "Rasuwa", district)) |> 
  filter(is.finite(individuals_reached)) |> 
  group_by(district, municipality, adm3_pcode) |> 
  summarise(beneficiary_frequencies = round(sum(individuals_reached, na.rm = TRUE)), 
            .groups = "drop") |> 
  left_join(
  ...
  relocate(impacted_buildings, .before = beneficiary_frequencies) |> 
  left_join(
    differences |>
      filter(affected_20260923 == 1) |>
      group_by(district, municipality, adm3_pcode) |>
      summarise(
        population_2026 = sum(projection_2026_population_total, na.rm = TRUE)), 
    by = c("adm3_pcode", "district", "municipality")
  ) |> 
  pivot_longer(cols = impacted_buildings:population_2026, 
               names_to = "variable",
               values_to = "value") |> 
  mutate(variable = case_when(
    variable == "population_2026" ~ "Total population in Affected Wards", 
    variable == "impacted_buildings" ~ "Impacted Buildings",
    variable == "beneficiary_frequencies" ~ "Beneficiary Frequencies (5Ws)", 
    TRUE ~ NA_character_
  )) |> 
  mutate(variable = fct_relevel(variable, c("Impacted Buildings", 
                                            "Beneficiary Frequencies (5Ws)", 
                                            "Affected Population (Flash Appeal)"))) |> 
  mutate(
    municipality = ifelse(municipality %out%
                            c("Bidur", "Gosaikunda", "Benighat Rorang", "Kispang", "Uttargaya",
                                      ...
                                      "Shahid Lakhan"), 
                          "Other", municipality)) |> 
  mutate(district = ifelse(municipality == "Other", "Other", district), 
         district = fct_relevel(district, 
                                c("Dhading", "Nuwakot", "Rasuwa", "Other"))) |> 
  group_by(district, municipality, variable) |> 
  ...
  labs(x = "", y = "", fill = "", 
       ...
       subtitle = "Beneficiary frequencies include double counting and are to used to visualise how resources have been allocated", 
       caption = "Source: NDRRMA RDNA; UN Nepal and its humanitarian partners; 5Ws 20260923.")

#ggsave("./plots/comparison_between_municipalities.png", height = 9, width = 8, units = "in")
```


<br>

The NDRRMA's stated affected population of 32,963 is solely derived from the number of persons affected by impacted buildings. As mentioned, calculations of a larger figure that also includes populations who have been cut off from essential services and income-earning opportunities are currently underway. 

The map below shows individuals reached in the 5Ws with OCHA's formula for avoiding double counting applied. Though the scale of the response in Bidur is very large, and District authorities have indicated that the municipality is saturated, the total number of individuals reached is only about double what it is in Gosaikunda.

As a final note, the 5Ws only captures the response achievements of UN Nepal and its humanitarian partners. For a more comprehensive understanding of gaps, data on the government's response is being sought. 


<br>

[![Click map for full-size version](./plots/municipal_max.png)](https://raw.githubusercontent.com/nepal-rasuwa-trishuli-floods-undac/nepal_floods_general/main/plots/municipal_max.png)
...
Bearing these limitations mind, the plots below shows ward-level 5W achievements. We have used the sum of all beneficiary frequencies reached, without regard for double counting, as it provides an idea how agencies have allocated their resources to date.

On the x-axis of the plots below is the number of beneficiary frequencies reached per ward. On the y-axis is the number of beneficiary frequencies, as a % of the ward's population. The size of each point indicates each ward's share of damaged buildings: as with the municipal-level comparisons, the scale of response interventions does not always align with the scale of damage in each area.  
...
consolidated |>
  ...
  mutate(district = ifelse(district %out% c("Dhading", "Nuwakot", "Rasuwa"), "Other", district),
         district = fct_relevel(district, 
                                c("Dhading", "Nuwakot", "Rasuwa", "Other")), 
         ward_label = paste0(municipality, "-", ward)) |> 
  replace_na(list(impacted_buildings = 0)) |> 
  ggplot(aes(x = individuals_reached, y = pc_reached)) + 
  geom_point(aes(colour = district, size = impacted_buildings), 
             alpha = .8) +
  geom_text_repel(aes(label = ward_label), 
            size = 1.5, alpha = .8) +
  ...
  labs(x = "Beneficiary frequencies 5Ws (log scale)", 
       y = "Beneficiary frequencies as % of ward population", 
       colour = "District", 
       size = "Impacted buildings", 
       title = "The response to-date has been focused on a small set of highly-impacted areas", 
       subtitle = "Beneficiary frequencies (which include double counting) are used because they are a good indicator\nof how resources have been allocated.") + 
  guides(
...
consolidated |>
  ...
  labs(x = "Beneficiary frequencies 5Ws (log scale)", 
       ...
       colour = "District", 
       size = "Impacted buildings", 
       title = "Sub-section of the plot above", 
       subtitle = "Showing wards where beneficiary frequencies are less than 200% of the ward population") + 
  guides(
    colour = guide_legend(override.aes = list(alpha = 1, size = 3), 
                          order = 1), 
    size = guide_legend(order = 2)
  ...
  plot_annotation(caption = "Source: 5Ws 20260923, National Statsics Office Nepal")

# ggsave("./plots/beneficiary_frequencies_by_ward.png", height = 10, width = 8.5, units = "in")
...
There is a very high concentration of beneficiary frequencies in Bidur-2 and Bidur-2, likely due to their relative accessibility and the presence of several holding centres within those wards. 

This is also evident in the map of beneficiary frequencies in the map below. 

<br>

[![Click map for full-size version](./plots/ward_reached_sum.png)](https://raw.githubusercontent.com/nepal-rasuwa-trishuli-floods-undac/nepal_floods_general/main/plots/ward_reached_sum.png)


<br>


Also from the map, we note a non-insignificant amount of humanitarian assistance that are outside of the affected areas. These are summarised in the table below. Some of these interventions were in more distant holding centres, like those in Kathmandu, but this will need to be investigated in more detail. 
...
consolidated |> 
  ...
  mutate(ward_category = ifelse(affected_20260923 == 1, "Affected area", "Outside affected area")) |> 
  ...
  rename(`Ward category` = ward_category, 
         ...
         `% of total` = pc) |> 
  kable(caption = "Beneficiary frequencies by type of ward, 5Ws 20260923") |> 
...
[![Click map for full-size version](./plots/ward_sectors.png)](https://raw.githubusercontent.com/nepal-rasuwa-trishuli-floods-undac/nepal_floods_general/main/plots/ward_sectors.png)

<br>

Sector presence is densest in Nuwakot. Though even within municipalities as well covered as Bidur, there is a marked difference when it comes to sector/Cluster presence on the western bank of the river. 
...
ward_summary |>
  pivot_longer(
    cols = food_security_individuals_reached:cccm_individuals_reached, 
    names_to = "sector", 
    values_to = "frequencies"
  ) |> 
  mutate(sector = str_remove_all(sector, "_individuals_reached")) |> 
  filter(!is.na(frequencies)) |> 
  mutate(count = 1) |> 
  group_by(sector, district, municipality, ward, adm3_pcode) |> 
  summarise(count = sum(count), .groups = "drop") |> 
  mutate(ward_label = paste0(district, ", ", municipality, "-", ward)) |> 
  mutate(sector = str_replace_all(sector, "_", " "), 
         sector = str_to_title(sector), 
         sector = ifelse(sector == "Cccm", "CCCM", sector), 
         sector = ifelse(sector == "Health Nutrition", "Health & Nutrition", sector), 
         sector = ifelse(sector == "Wash", "WASH", sector),
         sector = str_trim(sector)) |> 
  filter(sector != "Logistics") |> 
  ggplot(aes(x = count, y = ward_label)) + 
  geom_col(aes(fill = sector)) + 
  scale_fill_manual(values = c(
    "CCCM" = "#999999", 
    "Shelter" = "#ed1847",
    "Food Security" = "#72bf44", 
    "Education" = "#ffc800", 
    "Protection" = "#f58220",
    "WASH" = "#009edb",
    "Health & Nutrition" = "#a05fb4")
  ) +
  scale_x_continuous(breaks = seq(1, 7, 1)) +
  labs(title = "Sector presence by Ward", 
       subtitle = "According to the 5Ws on 20260923", 
       x = "Number of sectors", 
       y = "", 
       fill = "") 

#ggsave("./plots/sector_presence_by_ward.png", height = 5, width = 7, units = "in")
...
[![Click map for full-size version](./plots/assessment_map.png)](https://raw.githubusercontent.com/nepal-rasuwa-trishuli-floods-undac/nepal_floods_general/main/plots/assessment_map.png)
...
Given the various methodologies being employed and differing sampling frames employed, there is no mathematical way to reconcile the results of these various assessments. However, more in-depth secondary data reviews should be possible for Bidur and Belkotgadhi in Nuwakot, Gosaikunda and Uttargaya in Rasuwa, and Galchhi and Gajuri in Dhading. However, reconciling planned and conducted assessments into the 4 sub-contexts mentioned in the next section will be much easier. 
...
[![Click map for full-size version](./plots/wards_clustered.png)](https://raw.githubusercontent.com/nepal-rasuwa-trishuli-floods-undac/nepal_floods_general/main/plots/wards_clustered.png)

<br>

The specific indicators used in the clustering were:
